...
The system uses an **O(N²) Greedy Insertion Heuristic** instead of an intensive O(N!) Permutation Calculator. This allows the system to infinitely scale, routinely bundling 5+ orders into a single delivery vector in milliseconds.
...
It utilizes a **Cascaded Concentric Ring Filter** (`drivers/selection.py`):
*   **Wave 1**: Broadcast to drivers mathematically < 2km away. Wait `wave_timeout_seconds` (e.g. 30s) for a response.
*   **Wave 2**: Automatically widen the radius to < 4km and push to new drivers.
*   **Wave 3, 4, 5**: Continuously widen the net until the maximum threshold allows the Job to be claimed. 
